...
```bash
helm upgrade sgi sgi-umbrella-0.1.47.tgz --install --namespace sgi-demo -f ./config/values.demo.yaml
```
...
```bash
helm upgrade sgi sgi-umbrella-0.1.47.tgz --install --namespace sgi-demo -f ./config/values.demo.yaml --set usr-service.image.tag==0.1.0
```
